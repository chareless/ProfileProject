    ...
    if (if_layout) {
        ...
        preset_layout.forEach(function (element) {
            element.addEventListener('click', function () {
                ...
                saveLayoutToSession(layoutValue, "Layout")
...
function layout_theme_contrast_change(value) {
    ...
    saveLayoutToSession(value, "ThemeContrast");
}
...
function layout_caption_change(value) {
    ...
    saveLayoutToSession(value,"CaptionShow")
}
...
function preset_change(value) {
    ...
    if (control) {
        ...
        saveLayoutToSession(value, "PresetTheme");
    }
}
...
function layout_rtl_change(value) {
    ...
    saveLayoutToSession(value,"RtlLayout")
}
...
function layout_change(layout) {
    ...
    // Send the selected layout to the server to save in the session
    saveLayoutToSession(layout,"DarkLayout");
}
...
function updateLogos(color) {
    ...
    if (document.querySelector('.navbar-brand .logo-lg')) {
        document.querySelector('.navbar-brand .logo-lg').setAttribute('src', logoSrc);
    }
    if (document.querySelector('.auth-main .auth-wrapper.v1 .text-center .logo-lg')) {
        document.querySelector('.auth-main .auth-wrapper.v1 .text-center .logo-lg').setAttribute('src', logoSrc);
    }
    if (document.querySelector('.auth-main .auth-wrapper.v2 .text-center .logo-lg')) {
        document.querySelector('.auth-main .auth-wrapper.v2 .text-center .logo-lg').setAttribute('src', logoSrc);
    }
    if (document.querySelector('.auth-main.v1 .auth-sidefooter')) {
        document.querySelector('.auth-main.v1 .auth-sidefooter img').setAttribute('src', logoSrc);
    }
    if (document.querySelector('.footer-top .footer-logo')) {
        document.querySelector('.footer-top .footer-logo').setAttribute('src', logoSrc);
    }
}

function saveLayoutToSession(layout, type) {
    if (type == "DarkLayout") {
        if (layout == "dark")
            layout = "true";
        else
    ...
    fetch('/Home/SaveLayout', {
        ...
        body: JSON.stringify({ layout: layout, type: type })
    })
...
function resetLayoutToSession() {
    fetch('/Home/ResetLayout', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        ...
        .then(data => {
            ...
            location.reload(); // Doðru þekilde çaðrýldý
        })
        .catch(error => console.error('Error saving layout:', error));
}
...
function change_box_container(value) {
    if (document.querySelector('.pc-content')) {
        ...
        saveLayoutToSession(value,"BoxContainer")
    }
}
